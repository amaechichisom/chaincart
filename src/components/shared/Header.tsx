...
const MobileMenu: React.FC<IMobileMenu> = ({
  ...
  const isSeller =
    location.pathname.includes("seller") || location.search.includes("seller");
                ...
                <ul className="flex flex-col gap-4 text-lg ">
                  {menu.map((item, index) => (
                    <Link
                      ...
                      className={`!text-white hover:!underline cursor-pointer text-sm py-2 px-4 rounded-2xl ${
                        item.href === location.pathname ? "bg-[#B9E8FE]" : ""
                      }`}
                    >
                      {item.name}
                    </Link>
  ...
  return (
    <section className="p-3 px-4 flex items-center justify-between gap-2 lg:container lg:mx-auto">
      <div className="flex items-center gap-2">
        <Logo size="small" />
        <h3 className="font-manrope text-xl text-warp-100 font-bold">
          ChainCart
        </h3>
      </div>
        ...
        <nav className={`hidden lg:flex mt-2 w-full mx-auto`}>
          <ul
            className={`flex gap-6 w-full ${
              isSeller ? "justify-center" : "justify-end"
            }`}
          >
            {menu.map((item, index) => (
              <li key={index}>
                ...
                ${
                  item.href === `/category?search=${search}`
                    ? "bg-[#B9E8FE]"
                    : "bg-neutral-100"
                }`}
                >
